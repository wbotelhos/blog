...
describe Mailer do
  ...
  context "content parts" do
    it "should have the text body" do
      ...
      body.should match(%r[O artigo <strong><a href="#{CONFIG['url_http']}/articles/#{article.id}" style="color: #277DA8; text-decoration: none;">#{article.title}</a></strong> no qual voc&ecirc; participou foi respondido por <strong>#{new_comment.name}</strong>.])
      body.should match(%r[N&atilde;o perca a discuss&atilde;o, visite: <a href="#{CONFIG['url_http']}/articles/#{article.id}#comment-#{new_comment.id}" style="color: #277DA8; text-decoration: none;">http://#{CONFIG["url"]}/articles/#{article.id}#comment-#{new_comment.id}</a>])
      body.should match(%r[N&atilde;o perca a discuss&atilde;o, visite: <a href="#{CONFIG['url_http']}/articles/#{article.id}#comment-#{new_comment.id}" style="color: #277DA8; text-decoration: none;">http://#{CONFIG["url"]}/articles/#{article.id}#comment-#{new_comment.id}</a>])
    ...
    it "should have the html body" do
      ...
      body.should include(%[Não perca a discussão, visite: #{CONFIG['url_http']}/articles/#{article.id}#comment-#{new_comment.id}])
      body.should include(%[---])
